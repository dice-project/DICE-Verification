...
public class SparkVerificationLaunchShortcut implements ILaunchShortcut {
		...
		if (model == null) {
			DiceLogger.logError(DiceVerificationUiPlugin.getDefault(),
					MessageFormat.format(Messages.VerificationLaunchShortcut_unexpectedArgError,
							DiceVerificationPlugin.VERIFICATION_LAUNCH_CONFIGURATION_TYPE_SPARK, model));
			return;
		}

		if (!UML2ModelHelper.hasProfileApplied(model.getURI().toString(), DiceProfileConstants.SPARK_PROFILE_NAME)) {
			DiceLogger.logError(DiceVerificationUiPlugin.getDefault(),
					MessageFormat.format(Messages.VerificationLaunchShortcut_missingProfileMessage,
							DiceVerificationPlugin.VERIFICATION_LAUNCH_CONFIGURATION_TYPE_SPARK,
	...
	protected ILaunchConfiguration findLaunchConfiguration(UmlModel model, String mode) throws CoreException {
		...
		ILaunchConfigurationType verLaunchConfigurationType = launchManager
				.getLaunchConfigurationType(DiceVerificationPlugin.VERIFICATION_LAUNCH_CONFIGURATION_TYPE_SPARK);
		...
		launchConf.setAttribute(VerificationLaunchConfigurationAttributes.TIME_BOUND, 20);
		launchConf.setAttribute(VerificationLaunchConfigurationAttributes.HOST_ADDRESS, DiceVerificationUiPlugin
				.getDefault().getPreferenceStore().getString(PreferenceConstants.HOST.getName()));
		launchConf.setAttribute(VerificationLaunchConfigurationAttributes.PORT_NUMBER, DiceVerificationUiPlugin
